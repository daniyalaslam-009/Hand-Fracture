...
if uploaded_file is not None and model_loaded:
    ...
    confidences = detections.conf.cpu().numpy() if num_fractures > 0 else []
    avg_confidence = float(np.mean(confidences)) if num_fractures > 0 else 0.0
    
    with col2:
        st.markdown('<div class="result-section">', unsafe_allow_html=True)
        st.markdown("#### 🩻 Analysis Results")
        if num_fractures > 0:
            st.markdown(f'<div class="success-box">✅ <strong>{num_fractures} fracture(s) detected</strong></div>', unsafe_allow_html=True)
    ...
    with col_m1:
        st.markdown(f"<div class='metric-card'><h3>Fractures</h3><p>{num_fractures}</p></div>", unsafe_allow_html=True)

    with col_m2:
        st.markdown(f"<div class='metric-card'><h3>Avg Confidence</h3><p>{avg_confidence:.1%}</p></div>", unsafe_allow_html=True)

    with col_m3:
        st.markdown(f"<div class='metric-card'><h3>Threshold</h3><p>{confidence:.0%}</p></div>", unsafe_allow_html=True)

    with col_m4:
        ts = datetime.now().strftime("%H:%M:%S")
        st.markdown(f"<div class='metric-card'><h3>Scan Time</h3><p>{ts}</p></div>", unsafe_allow_html=True)
...
Total Fractures: {num_fractures}
Average Confidence: {avg_confidence:.1%}
Threshold: {confidence:.0%}

"""
        st.download_button("📄 Download Report", report,
            file_name="fracture_report.txt", mime="text/plain",
    ...
    st.error("❌ Model not loaded. Ensure 'best.pt' is in the app directory.")
else:
    st.markdown("""
    <div class="info-box">
    <strong>Welcome!</strong> Upload an X-ray image to begin analysis.
    </div>
    """, unsafe_allow_html=True)
